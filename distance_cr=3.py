...
g_list = [];
g_bin_list = [];
data_file = 'data/data_k=' + str(memory) + '_cr=3.dat';
...
for ind in range(len(g_bin_list)):
	for jnd in range(ind+1,len(g_bin_list)):
		for knd in range(jnd+1,len(g_bin_list)):
			distance = 0;
			for j in range(len(v_slice0)):
				...
				for i in range(len(g_bin_list[0])):
					...
					path3 += int(v_slice0[j][i])*int(g_bin_list[knd][i]);
				path1 = path1 % 2;
				path2 = path2 % 2;
				path3 = path3 % 2;
				weight = path1 + path2 + path3;
			...
			for i in range(len(g_bin_list[0])):
				...
				path3 += int(first_point[i])*int(g_bin_list[knd][i]);
			weight1 = (path1 % 2) + (path2 % 2) + (path3 % 2);
			...
			f.write("Constraint length (K): {}. Generator polynomials: {}, {}, {}. Distance: {}  ".format(memory, g_bin_list[ind],g_bin_list[jnd], g_bin_list[knd], distance));
			f.write('\n');
